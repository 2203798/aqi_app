...
def analyze():
    ...
    predicted_aqi = int(random_forest_regression_model(convert_2D_array(inputs), aqi_X_train, aqi_y_train)[0])
    return render_template("index.html", aqi=predicted_aqi,
                            aqi_equiv=aqi_bucket(predicted_aqi),
                            time_series=time_series,
                            ts_metrics = ts_metrics,
                            tree_metrics = random_forest_regression_model(convert_2D_array(inputs), aqi_X_train, aqi_y_train)[1],
...
station_hour = 'https://drive.google.com/u/1/uc?id=1ZshXKxm69nzamvj29Gn9bE4IGzjwu2FI&export=download&confirm=t&uuid=3458be04-41af-48d9-bb4d-e4e2cf50b69c'
station_hour_final = 'https://drive.google.com/u/1/uc?id=1tmObFQNEknM9C2IPqYbS6iHH3PjRzFsl&export=download'
station_day = 'https://drive.google.com/u/1/uc?id=1mWXUiqzz1e98q_WEss29ry6ppSOQbo1K&export=download'
city_hour = 'https://drive.google.com/u/1/uc?id=1_MLdwmJ3I4RGEvmBestLONH_N4hvw5zn&export=download'
city_day = 'https://drive.google.com/u/1/uc?id=1tm8KcU8If6n29MFY91yl-NZfh4rzwhpf&export=download'
stations = 'https://drive.google.com/u/1/uc?id=1XXXfzei7-KSAivKml9PlYXRIR-sdDsoz&export=download'

PATH_STATION_HOUR = station_hour
PATH_STATION_DAY = station_day
PATH_CITY_HOUR = city_hour
PATH_CITY_DAY = city_day
...
df= pd.read_csv(station_hour_final)

df['Datetime'] = pd.to_datetime(df['Datetime'])

cities_all = df.pivot_table(values='AQI', index=['Datetime'])
cities=cities_all.resample(rule='MS').mean()

diff = cities['AQI'].diff(periods=1)
diff.dropna(inplace=True)

from statsmodels.tsa.seasonal import seasonal_decompose
fig = seasonal_decompose(diff, model='additive')

#dividing into train and test:
train_data=cities[:'2018-12']
test_data=cities[:'2019-12']

#Building the model:
model=SARIMAX(train_data,order=(0,0,0),seasonal_order=(1,0,1,12), trend='n')
results=model.fit()
...
lower_limits = confidence_intervals.loc[:,'lower AQI']
upper_limits = confidence_intervals.loc[:,'upper AQI']
...
test= cities['AQI']['2019-01':'2019-12']
...
ts_metrics = [RMSE, mape, r2_score]

test=cities['AQI']['2019-01':'2019-10'] #actual
forecasts = results.get_forecast(steps=31, dynamic=True)
prediction = pd.DataFrame(forecasts.predicted_mean) #predicted
prediction.columns = ['predicted_AQI']
index = prediction.index.strftime(date_format="%Y-%m-%d")


labels = index.values
predicted = prediction['predicted_AQI'].values
actual = test.values

time_series = [labels.tolist(), predicted.tolist(), actual.tolist()]
